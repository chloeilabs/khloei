...
/** One page in Khloei's shared browser context. */
...
/** The tab strip and the page all subsequent browser actions target. */
...
export type OpenTabInput = { url: string };
export type SwitchTabInput = { tabId: string };
...
export type ControlState = {
  ...
  requested: boolean;
  /** When the Bot asked, so an unanswered request can expire. */
  requestedAt?: string;
  /** Label for a secret the Bot asked the person to enter. Never its value. */
  secretWanted?: string;
  /** The field ref that receives a pending secret. Kept on the computer service. */
  secretRef?: string;
  /** Snapshot generation associated with the pending secret request. */
